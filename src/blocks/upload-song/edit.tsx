...
const SongUploadBlock: React.FC< SongUploadProps > = ( {
	...
	const ALLOWED_MEDIA_TYPES = allowedMimeTypes.length
		? allowedMimeTypes
	...
	const handleError = async ( e ) => {
		...
		setError(
			__(
				'This feature is only usable on the front end of the website.',
				'upload-block'
			)
		);
	};
	...
	const fileTypeLabel = fileTypeOptions
		.filter( ( option ) => allowedMimeTypes.includes( option.value ) )
		.map( ( option ) => option.label );
	const allowedMimeTypesLabel = fileTypeLabel.join( '|' );

	const maxFileSizeLabel = `${ __(
		'Allowed file types: ',
		'upload-block'
	) } ${ allowedMimeTypesLabel } ${ maxFileString }`;
	return (
		<section { ...blockProps }>
			<div className="song-upload-max-width-container">
				<InspectorControls>
					<PanelBody
						title={ __( 'Heading Settings', 'upload-block' ) }
					>
						<SelectControl
					...
					<PanelBody
						title={ __( 'File Restrictions', 'upload-block' ) }
					>
						<SelectControl
							multiple
							...
							onChange={ ( value: string[] ) =>
								setAttributes( { allowedMimeTypes: value } )
							}
						/>
						<RangeControl
				...
				<div className="song-upload-form wp-block-form">
					<div className="song-upload-form__column">
						<span className="song-upload-form__label">
							<span className="song-upload-form__label-content wp-block-form-input__label-content">
								{ maxFileSizeLabel }
							</span>
						</span>
						<label
							htmlFor="song-file"
							className="song-upload-form__input-file-button wp-block-button"
						>
							<span className="button button-primary wp-block-button__link wp-element-button">
								Select song
							</span>
						</label>
						<span className="song-upload-form__file-selected">
							No file chosen
						</span>
						<input
					...
					<div className="song-upload-form__column">
						<label
							htmlFor="song-title"
							className="song-upload-form__label"
						>
							<span className="song-upload-form__label-content wp-block-form-input__label-content">
						...
						<TextControl
							...
							value={ songTitle }
							onChange={ ( value ) =>
								setAttributes( { songTitle: value } )
							}
							placeholder={ __(
								'Song Title (Input on front end only)',
								'upload-block'
							) }
							disabled={ true }
